...
buildModel <- function()
{
		...
		blacklist = read.csv('blacklist_bayesiannetwork.csv')
...
doRecommendation <- function()
{
	...
	#for(i in 1:1)
	{
		...
		#for(j in 1:500)
		{
			...
			mytestrow[,grep("MED.",names(mytestrow))] = 0
			#mytestrow[i,grep("MED.",names(mytestrow))] = 0
			mytestrow[i,medicationshere] = 1
	    ...
	    #cat(paste("R==",ReadmitRiskScore,"L==",LOSRiskScore,"M==",MortalityRiskScore,"\n"))
	...
	outputdf = data.frame(MedicationCombination,ReadmitRiskScore,LOSRiskScore,MortalityRiskScore)
